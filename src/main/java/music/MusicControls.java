...
class MusicControls extends MusicObject
{
    ...
    MusicControls(final MessageReceivedEvent event, final AudioPlayerManager playerManager, final MusicManager musicManager) {
    ...
    void playSong(final User author) {
    ...
    void nowPlaying() {
        if (isMusicPlaying()) {
            final AudioTrack track = musicManager.getScheduler().getCurrentTrack();
    ...
    private boolean isUrl(final String songQuery) {
        final Pattern pattern = Pattern.compile(URL_REGEX);
        final Matcher matcher = pattern.matcher(songQuery);
        return matcher.find();
    }

    private String buildYouTubeQuery(final String keyphrase) {
