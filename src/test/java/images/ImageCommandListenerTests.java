...
public class ImageCommandListenerTests
{
    ...
    public void getImage_GivenProperSearchQuery_ReturnsURL()
    {
        ...
        final String resultURL = "google.com";

        when(channel.sendMessage(anyString())).thenReturn(messageAction);
        when(contentManager.getImgurGallery(anyString())).thenReturn(gallery);
        when(gallery.getData()).thenReturn(dataList);
        when(dataList.isEmpty()).thenReturn(false);
        when(dataList.get(anyInt())).thenReturn(data);
        when(data.getLink()).thenReturn(resultURL);

        assertEquals(resultURL, imageCommandListener.getImage(query, channel, contentManager));
    }
